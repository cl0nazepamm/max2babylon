...
namespace Max2Babylon
{
    internal partial class BabylonExporter : ILoggingProvider
    {
        ...
        // The legacy flattening flow only inspects the first child when it needs to
        // recurse from a blocking node. Keep that behavior explicit so Roslyn does
        // not flag the old loop-and-return pattern as unreachable.
        private bool ContinueFlattenabilityFromFirstChild(IINode node, AnimationGroupList animationGroupList, ref List<IINode> flattenableNodes)
        {
            ...
            IINode firstChild = node.GetChildNode(0);
            return IsMeshFlattenable(firstChild, animationGroupList, ref flattenableNodes);
        }
        ...
        private bool IsMeshFlattenable(IINode node, AnimationGroupList animationGroupList,ref List<IINode> flattenableNodes)
        {
            ...
            if (node.IsRootNode)
            {
                return ContinueFlattenabilityFromFirstChild(node, animationGroupList, ref flattenableNodes);
            }
